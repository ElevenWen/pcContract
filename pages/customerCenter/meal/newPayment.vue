<template>
    <div id="newPayment" class="commonWrap">
        <!-- 公共头部 -->
		    ...
		    <div class="ContensWrap">
		        ...
		        <div class="bodyContent">
		            ...
		            <div class="productInfoWrap">
		                ...
		                <span class="money">¥{{payable.toFixed(2)}}</span>
		            ...
		            <div class="detailWrap" v-if="isShowDetail">
		                ...
		                <div class="detailContent" v-for="item in orderDetail.OrderDetails">
		            ...
		            <div class="paymentWrap">
		                <ul class="pay-list">
		                	...
		                	<li v-for="(data,i) in payArr" v-if="data.isisBalacne">
		                		<div class="img forbidden" v-if="data.type==0 && data.AccountBalance<payable"></div>
			                    ...
			                    <span class="noPay" v-if="data.type==0 && data.AccountBalance<payable">可用余额不足</span>
			                    <!--余额才有-->
...
<script>
	...
	import { payment_wxCode,urlShortPort,websitePort  }  from  '~/store/ajaxAPI/perPort.js'
	...
	export default {
	    data(){
	        return {
	            ...
	            title:"套餐列表",
	            payable:0,//应付金额
	            website:'/customerCenter/meal/mealList'
	        }
	    },
	    components:{
	        leftNav,
	    ...
	    mounted(){
	    	...
	    	if(this.$route.query.type == 1){
	    		this.website = '/customerCenter/meal'
	    	}
			...
			.then((res)=>{
				...
				this.orderLength = 1;
				this.orderDetail.OrderDetails.forEach((val)=>{
					this.payable += Number(val.Price);
				})
	    ...
	    computed:{
	    	//支付方式选择后应付
	    	totalMoney(){
	    		let tempPayTotalMoney = (this.payable*10000 - this.usableCoin*10000)/10000;
	    	...
	    	balanceMoney(){
	    		//是否选择记账币
				var m = this.payable-this.usableCoin;
	    ...
	    methods:{
	    	...
	    	requestData(){
		    	//支付方式列表
		    	var params = {
		    		orderNum:this.orderDetail.OrderNumber
		    	}
		    	...
		    	.then((res)=>{
		    		res.data.list.forEach((val)=>{
		    			switch (val.Type){
		    				case 0:
		    					this.payArr.push({way:'支付宝支付',icon:'/customerCenter/alipay-mode.png',type:2,isisBalacne:true});
		    					break;
		    				case 1:
		    					this.payArr.push({way:'微信支付',icon:'/customerCenter/wx-mode.png',type:1,isisBalacne:true});
		    					break;
		    				case 2:
		    					this.payArr.push({way:'中国银联',icon:'/customerCenter/unionpay-mode.png',type:3,isisBalacne:true});
		    					break;
		    				case 3:
		    					//若记账币不可用才判断余额是否可用
		    					if(!this.isShowCoin){
		    						//判断默认支付方式
			    					if(!val.IsisBalacne || val.AccountBalance<this.payable){
			    						//默认微信
		    					...
		    					this.payArr.push({way:'账户余额',icon:'/customerCenter/banlance-mode.png',type:0,isisBalacne:val.IsisBalacne,AccountBalance:val.AccountBalance});
		    					break;
		    				case 5:
		    					//判断是否为代付页面跳转过来的
		    					if(this.isShowJZ){
		    						this.payArr.push({way:'找人代付',icon:'/customerCenter/paid-mode.png',type:4,isisBalacne:false});
		    					}else{
		    						this.payArr.push({way:'找人代付',icon:'/customerCenter/paid-mode.png',type:4,isisBalacne:true});
		    					}
	        ...
	        payImmediately(){
	        	...
	        	//记账币可用一次性支付并只选择记账币
	        	if(this.selectedJzb && this.payable <= this.usableCoin && this.currentType < 0){
	        		let params = {
	            			orderNo:this.orderDetail.OrderNumber,
	            		...
	            		.then((res)=>{
	            			this.$router.replace('/customerCenter/meal')
	            		})
	            		.catch((error)=>{
	            			// console.log(error)
	            		})
	    		...
	    		switch (this.currentType){
	            	...
	            	case 1://微信
		                ...
		                this.wxCode = `${payment_wxCode}/Payment/MakeActivityPayQRCode?out_trade_no=${this.orderDetail.OrderNumber}`;
	...
	//				    		 console.log("我是在 ",res,res.data.msg)
					    		if(res.data.msg == "订单状态必须为待支付"){
					    			...
					    			that.$router.replace('/customerCenter/meal')
					    			return
					    			//自动跳转 我的订单界面
					    			...
					    			that.wxCode = `${payment_wxCode}/Payment/MakeActivityPayQRCode?out_trade_no=${this.orderDetail.OrderNumber}`;
					    		}
					    	});
	            	...
	            	case 2://支付宝
	            		...
	            		let data = {
			    			frontUrl:'/',//前端回调地址
			    			orderId:this.orderDetail.OrderNumber,//订单号
	            		...
	            		break;
	            	case 3://银联
	            		let orderId = this.orderDetail.OrderNumber;
			    		let dfUnionPay = this.isShowJZ ? 1 : 0 ;
			    		let data1 = {
			    			frontUrl:`https://ec.wqbol.com/customerCenter/meal`,//前端回调地址
	            	...
	            	case 4:
	            		//找人代付
	            		location.href = `${websitePort}/cart/paid?orderNum=${this.orderDetail.OrderNumber}&type=1`;
	            		break;
	            	default:
	            		break;
	            }
...
<style lang="less" scoped>
	@import '~assets/less/customerCenter/newPayment.less';
</style>
